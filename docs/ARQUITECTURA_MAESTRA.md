...
3. Los efectos colaterales cruzados viajan por **eventos de dominio**, nunca por escritura directa: el POS no escribe en finanzas ni en inventarios; emite `CuentaPagada`, `ItemComandado`, y los listeners de cada módulo reaccionan.
   - **Una PREGUNTA que cruza módulos también vive en el kernel, como interfaz** (D266, D277). Van dos en dos pasos consecutivos —`LiveServiceProbe` para el salón, `CashSessionProbe` para las finanzas—, así que no es un caso particular sino la forma normal de que un módulo de dominio necesite algo que sólo sabe uno operativo: `Floor` necesita saber si una mesa tiene servicio en curso para negarse a liberarla, y la respuesta la sabe `Pos` — que ya depende de `Floor`. El contrato es una interfaz del kernel que `Floor` consume y `Pos` implementa, con la dependencia invertida. Un evento no sirve aquí: no se puede preguntar con un anuncio, y la respuesta hace falta antes de escribir.
...
- **La suite corre en PARALELO, salvo un grupo declarado** (Iteración 4): `composer test` lanza `artisan test --parallel --exclude-group=serial` y después `--group=serial`. Pasó de **20 minutos a 6**, con las mismas 909 pruebas. El grupo `serial` existe porque dos clases de prueba salen del marco de `RefreshDatabase` y no pueden compartir base con otro proceso: la suite `Concurrency`, que abre una segunda conexión a mano, y la purga del sembrador de demostración, que corre comandos de Artisan sobre la conexión por omisión. En paralelo producían deadlocks de MySQL que no decían nada sobre el código. `composer test:serie` conserva la corrida completa en un solo proceso para cuando haga falta comparar.
- **`memory_limit` de 512M en `phpunit.xml`** (Iteración 4): la suite dejó de caber en los 128 MB por omisión al llegar a ~890 pruebas, y reventaba **abortando la corrida completa** en lugar de fallar una prueba. Antes de subir el límite se corrigió un candado que concatenaba todo `app/` en una cadena, pero el techo se alcanzaba igual: es el tamaño del proyecto. No `-1`, porque un límite alto sigue avisando de una fuga real.
- **Suite `Concurrency`** (Iteración 3): pruebas que necesitan **dos conexiones de verdad**, y que por eso no pueden vivir en `Feature` — `RefreshDatabase` envuelve cada prueba en una transacción y una transacción hace los datos invisibles para cualquier otra conexión. Hacen `COMMIT` y limpian a mano. Sólo van ahí las que no se pueden escribir de otra forma: hoy, que el registro del kardex **bloquea** la fila del saldo mientras calcula. Se observa desde el evento `created` del modelo, que Eloquent dispara dentro de la transacción del servicio; es el único instante en que un lock de milisegundos es observable.
- **Verificación en NAVEGADOR, obligatoria antes de dar por hecha una pantalla** (Iteración 3, paso 11): la suite en verde no basta. Con 842 pruebas verdes, abrir el navegador encontró cinco defectos vivos —incluido uno que respondía 422 a una confirmación que sí había ocurrido, con la mercancía ya en el kardex— y ninguno era detectable con pruebas de API: dependían de carreras de carga, de datos viejos de una versión anterior, o de que un efecto posterior al commit fallara. Ver D219 y D220. La segunda tanda de pantallas encontró otros cinco (D223–D226, D229), entre ellos un contrato del servidor que el cliente descartaba y un diálogo de autorización sin salida. Cada defecto encontrado así se cierra con su prueba de regresión cuando puede tenerla.
- **En desarrollo NO corre `queue:work`, así que ningún efecto asíncrono ocurre** (Iteración 3, paso 11): el recosteo en cascada, el descuento de inventario del POS y las proyecciones esperan en la cola indefinidamente. Cualquier verificación en el navegador que dependa de ellos parecerá un defecto del sistema; con 38 trabajos encolados, el costo de un producible se veía «mal» y estaba simplemente pendiente (D229). Antes de declarar un defecto de costeo o de inventario, mirar `jobs`.
- **Comando `comandia:permissions:sync`** (Iteración 3, paso 11): los permisos del catálogo cerrado se siembran al dar de alta el negocio, así que un permiso agregado en una iteración posterior **no existe** para las instalaciones que ya corrían — y su ruta devuelve 403 para todo el mundo. Hay que correrlo al actualizar. La suite no puede detectarlo: cada prueba provisiona un negocio nuevo con el catálogo del día (D219).
- **Candado de oyentes registrados** (Iteración 3, paso 10): un oyente sin `Event::listen` **no falla, no corre** — el efecto que debía producir simplemente no ocurre y en producción no se ve ningún error. Importa desde que confirmar una recepción tiene tres efectos en tres oyentes, dos de ellos en otros módulos. Y al revés: un evento que nadie despacha es código muerto que parece vivo. `tests/Architecture/ListenersAreRegisteredTest.php`.
- **Candado de modelos releídos** (Iteración 3, paso 10): un servicio no devuelve el resultado de `create()` sin `refresh()`. Eloquent devuelve el atributo **asignado** y no el almacenado, así que un `DECIMAL(12,4)` vuelve sin sus decimales y una **columna generada** —`variance`, `balance_after`, `transit_difference`— no vuelve en absoluto. El defecto apareció cuatro veces antes de tener candado (D134, D149, y los pasos 4 y 8 de la Iteración 3). `tests/Architecture/CreatedModelsAreRefreshedTest.php`.
- **Candado de refs sin `.value` en plantillas** (Iteración 2; corregido en la Iteración 3, paso 11): `v-if="save.generalError"` es **siempre** verdadero, porque un Ref es un objeto; de ahí salen recuadros de error vacíos y permanentes. Vigila `generalError`, `fieldErrors` e `isEmpty`, y desde el paso 11 **sólo dentro del bloque `<template>`**: la premisa de que esos nombres sólo existen como refs caducó cuando `ApiError` expuso un getter `fieldErrors`, y el candado marcaba una lectura correcta. En el script el mismo error falla ruidosamente, así que restringirlo es precisión, no concesión (D227). `tests/Architecture/FrontendRefUnwrapTest.php`.
- **Candado de truncamiento de dinero** (Iteración 4, paso 7): `bcmath` **trunca** al bajar de escala, así que `bcsub($total, $base, 2)` sobre un `$base` calculado con escala 6 se queda un centavo corto —siempre hacia abajo, en el IVA que el ticket desglosa (D237)—. Lo notable es que la convención ya era correcta en todo el repositorio y estaba escrita en el encabezado de `Decimal` desde la Iteración 2: vivía sólo en el hábito, y la primera vez que escribí la operación en un módulo nuevo la rompí. Comprueba la **cascada** dentro de una función (una variable de escala alta usada después con escala 2) y prohíbe `bcdiv` a escala de dinero; **permite escala 0**, donde truncar es la operación pedida y no un descuido. `tests/Architecture/MoneyRoundingTest.php`.
- **`actingAsSpa` tira la sesión antes de autenticar** (Iteración 4, paso 8): `withSession()` MEZCLA en la sesión que el cliente de pruebas ya traía, así que autenticarse como un segundo usuario dentro de una misma prueba respondía 401 con todo en orden. Lo caro no era el 401: como «autentícate como otro» no funcionaba, la costumbre era preparar al usuario ajeno por modelos y no ejercitar nunca su camino HTTP — justo el camino que una prueba de autorización o de aislamiento existe para vigilar. Una limitación del ayudante estaba moldeando las pruebas hacia ser más débiles sin que se notara (D243). Es la segunda de esta familia en la iteración, después del `flushHeaders()` del paso 0.
- **Un agente de impresión habla con `actingAsPrintAgent()`, no con `withToken()`** (Iteración 4, paso 9): el cliente de pruebas arrastra las cabeceras, y el `Referer` que deja un `actingAsSpa` hace que Sanctum trate la petición del agente como si fuera un navegador — `AuthenticateSession` lanza `AuthenticationException` antes de que el agente llegue a autenticarse, y el 401 apunta tres capas más allá de donde viene (D250). El ayudante tira cabeceras y sesión, que es además lo que hace el cliente real: un agente no es un navegador. **Tercera** corrección del arnés en la iteración, con la misma causa que las dos anteriores.
- **El diario impone el SIGNO de cada asiento** (Iteración 4, paso 10): `naturalSign()` declaraba desde el paso 4 que un retiro en positivo es «el error más fácil de cometer», y aun así lo cometí al asentar el cambio de un cobro — un cajón que cuadra al revés, con la diferencia a nombre del cajero y nada que falle. `RecordFinancialMovement` ahora lo **rechaza** en lugar de documentarlo, y no lo corrige en silencio: aplicar el signo automáticamente escondería que quien asienta entendió mal el sentido del movimiento. Una reversa conserva el tipo con el signo contrario (D253).
- **El candado de fronteras ignora las CADENAS** (Iteración 4, paso 11): buscaba `App\Modules\X\` en el texto del archivo, y un nombre de clase escrito como cadena admite dos formas equivalentes de las que sólo una coincidía. Dos oyentes de `Finance` con el mismo comportamiento se trataban distinto según cómo se hubieran tecleado las barras (D261). Un `sourceType` es un dato —así registra ADR-004 la procedencia de un asiento—, no una llamada; lo que sí es dependencia es un `use`, un tipo o una llamada estática.
- **Un candado que depende del estado de la CONEXIÓN se prueba en `Unit`** (Iteración 4, paso 14): `RefreshDatabase` envuelve cada prueba de `Feature` en una transacción, así que `transactionLevel()` nunca vale 0 ahí y una comprobación de «exige transacción abierta» no puede fallar nunca. Se prueba con un doble de la conexión. Estaba advertido en el encabezado de `DocumentNumberAllocatorGuardTest` desde la Iteración 3 y aun así se repitió al escribir el segundo asignador (D270) — la advertencia escrita no impide repetir el fallo; sólo lo impide una prueba que falla sola.
- **El cobro se asienta por lo que ENTRÓ, no por el importe de la cuenta** (Iteración 4, paso 20): `payment` vale lo entregado —`importe + propina + cambio`, que es lo entregado por construcción— y la propina **no mueve el cajón por sí misma**, porque siempre viaja dentro de un cobro. Antes la entrada iba neta (196) y la salida bruta (−84), así que el cambio se descontaba dos veces y toda caja con cambio aparecía corta por su importe exacto. Ninguna prueba lo veía porque todas pagan exacto, y con cambio cero el asiento ni se crea (D295).
- **El corte se calcula del diario con `affects_cash_drawer`, no con una fórmula enumerada** (Iteración 4, paso 19): el efectivo esperado es `SUM(amount) WHERE pos_session_id = X AND affects_cash_drawer = 1`. Equivale a la fórmula de §6.5 y tiene una propiedad que aquélla no: **un tipo nuevo de movimiento entra solo**. La enumerada habría que editarla cada vez, y el día que se olvidara un tipo el arqueo daría de más sin que nada fallara (D286). Es donde termina de pagarse que la bandera se COPIE al asentar en lugar de leerse del método: cambiarla hoy no altera los cortes de ayer.
- **Candado de cobertura de la purga del demo** (Iteración 4, paso 20): toda tabla con `tenant_id` tiene que desaparecer con `--fresh`, y se comprueba contra `information_schema` en lugar de contra un tenant sembrado. El candado que ya existía corre la purga de verdad, y aun así dejó pasar **trece tablas**: las que se llenan **operando** están vacías en un tenant recién sembrado, así que su `DELETE` no choca con nada. Cuidado con el criterio: «cascadea del tenant» es **circular** —el `DELETE FROM tenants` final sólo funciona porque la lista ya vació todo antes, con 160 claves foráneas `RESTRICT` de por medio— y esa primera versión habría aceptado una lista vacía (D290). `tests/Architecture/DemoPurgeCoversTenantTablesTest.php`.
- **Candado de alcance por sucursal** (Iteración 4, paso 20): un controlador que resuelve una `Branch`, `Terminal` o `RestaurantTable` a partir de lo que manda el cliente tiene que comprobar `canOperateInBranch()`. El `tenant_id` no protege de esto —la sucursal ajena es del **mismo negocio** y llega como un modelo válido—, y el mecanismo existía sin usarse: `Authorize::authorize($permiso, ?int $branchId = null)` acepta la sucursal como argumento **opcional**, y ese `= null` es la forma que toma el olvido. Eran once endpoints. Ojo con dos precisiones que costaron una vuelta cada una: la **terminal** cuenta (es el caso que originó el hallazgo, y la primera versión del candado no lo veía), y los **filtros de lista** no (son consultas, y meterlos llena el candado de ruido) (D292). `tests/Architecture/BranchScopeIsAssertedTest.php`.
- **La autorización de un CANAL se prueba como la de un endpoint, y por omisión no se prueba** (Iteración 5): `phpunit.xml` fija `BROADCAST_CONNECTION=null` y `NullBroadcaster::auth()` **no consulta los canales** — devuelve un 200 vacío. Una prueba de «este canal se rechaza» pasa en verde con el guardián invertido, borrado o inexistente. Y hay una segunda mitad: `Broadcast::channel()` registra sobre la conexión que es la de omisión **al arrancar**, así que cambiarla en la prueba deja un registro vacío y entonces se rechaza TODO — los rechazos vuelven a pasar por el motivo equivocado. Hacen falta las tres cosas: cambiar el driver, purgar la conexión y volver a cargar `routes/channels.php` (D302).
- **Candado de filtros del frontend** (Iteración 5): cada `api.get('/ruta', {…})` se compara contra la lista blanca REAL del endpoint. Un filtro inventado responde 422 (D182) y ese 422 tumba el `Promise.all` de la pantalla entera: no se ve «este filtro no funciona», se ve la pantalla **en blanco** o cargando para siempre. Van tres veces en dos iteraciones (D294, D308). Ojo: su primera versión **se saltaba la primera llave de cada objeto** —donde suele ir el filtro que importa— y pasó en verde sobre el defecto que existe para atrapar. `tests/Architecture/FrontendFiltersExistTest.php`.
- **Candado de nombres de ayudantes** (Iteración 3, paso 6): los ayudantes de un archivo de Pest son **funciones globales de PHP**, así que dos archivos que declaren el mismo nombre no producen una prueba en rojo — producen un `Cannot redeclare` que **aborta la suite completa** antes de ejecutar nada. Y correr el archivo solo pasa en verde, así que el fallo aparece justo cuando la entrega ya se dio por buena. `tests/Architecture/TestHelperNamesAreUniqueTest.php`.
