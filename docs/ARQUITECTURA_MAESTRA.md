...
3. Los efectos colaterales cruzados viajan por **eventos de dominio**, nunca por escritura directa: el POS no escribe en finanzas ni en inventarios; emite `CuentaPagada`, `ItemComandado`, y los listeners de cada módulo reaccionan.
   - **Una PREGUNTA que cruza módulos también vive en el kernel, como interfaz** (D266): `Floor` necesita saber si una mesa tiene servicio en curso para negarse a liberarla, y la respuesta la sabe `Pos` — que ya depende de `Floor`. El contrato es una interfaz del kernel que `Floor` consume y `Pos` implementa, con la dependencia invertida. Un evento no sirve aquí: no se puede preguntar con un anuncio, y la respuesta hace falta antes de escribir.
